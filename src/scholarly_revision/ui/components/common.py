...
from scholarly_revision.ui.state import project_label, redact_exception, set_active_project
...
def render_project_selector(orchestrator: Any) -> Any:
    ...
    if not projects:
        st.sidebar.info('No registered projects in this workspace root.')
        return None
    by_label = {project_label(item): item for item in projects}
    selected = by_label[st.sidebar.selectbox('Project', list(by_label))]
    set_active_project(
